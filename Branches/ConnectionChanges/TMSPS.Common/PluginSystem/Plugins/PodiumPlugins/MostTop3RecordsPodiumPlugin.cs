...
namespace TMSPS.Core.PluginSystem.Plugins.PodiumPlugins
{
    public class MostTop3RecordsPodiumPlugin : LocalRecordsPluginPlugin
    {
        ...
        protected MostTop3RecordsPodiumPlugin(string pluginDirectory)
        ...
        private void Callbacks_EndRace(object sender, Communication.EventArguments.Callbacks.EndRaceEventArgs e)
        {
            RunCatchLog(() =>
            {
                ...
                {
                    entries = rankingAdapter.DeserializeListByMost(Settings.MaxEntriesToShow, 3).ConvertAll(position => new PodiumPluginUIEntry(position.Amount.ToString("F0", CultureInfo.InvariantCulture), position.Nickname));
                }
